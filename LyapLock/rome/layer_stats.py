...
def layer_stats(
    ...
    def get_ds():
        ...
        # raw_ds = {'train': raw_ds}
        # NOTE: datasets>=3 removed dataset-script support.
        # The Wikipedia dataset is script-based, so we must ensure we use a compatible
        # datasets version (pin in requests.txt) and load by canonical hub name.
        config = dict(wikitext="wikitext-103-raw-v1", wikipedia="20200501.en")[ds_name]
        raw_ds = load_dataset(ds_name, config)
