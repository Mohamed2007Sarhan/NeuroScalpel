...
| **6** | 💬 Neural Chat | Launch an isolated, zero disk-overhead chat UI to immediately test the patched model's modified knowledge |
...
- **Abliteration Engine** — Auto-compute orthogonal refusal vectors and safely ablate them dynamically (jailbreaking safety filters precisely)
- **Zero-Overhead Chat UI** — Talk to your edited AI instantly without saving or reloading large checkpoints; VRAM efficient memory isolation
...
5. **Neural Chat & Abliteration**  
   Run the Abliteration routine from the Dashboard bypassing model refusal vectors. Test the newly minted model interactively using the isolated **Chat UI**.
