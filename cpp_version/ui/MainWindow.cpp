...
static QString nvidiaApiKey()
{
    QString k = QString::fromLocal8Bit(qgetenv("NVIDIA_API_KEY"));
    if (k.isEmpty())
        k = QString::fromLocal8Bit(qgetenv("NVAPI_KEY"));
    if (k.isEmpty())
        k = QString::fromLocal8Bit(qgetenv("OPENAI_API_KEY"));
    return k;
}

static void addNvidiaDeepSeekExtraBody(QJsonObject& payload)
{
    QJsonObject chatTemplateKw;
    chatTemplateKw["thinking"] = true;
    QJsonObject extraBody;
    extraBody["chat_template_kwargs"] = chatTemplateKw;
...
void MainWindow::startPhase1(const QString& orderText) {
    ...
    const QString apiKey = nvidiaApiKey();
    if (apiKey.isEmpty()) {
        m_winMind->appendText(
            "[ERR] Missing NVIDIA_API_KEY (or NVAPI_KEY). Export it before launching the app.\n",
            "#ff003c");
        return;
    }
    ...
    addNvidiaDeepSeekExtraBody(payload);
...
void MainWindow::startPhase3(const QJsonObject& analysisDict) {
    ...
    if (apiKey.isEmpty()) {
        m_winMind->appendText(
            "[ERR] Missing NVIDIA_API_KEY (or NVAPI_KEY). Cannot run Phase 3.\n",
            "#ff003c");
        return;
    }
...
void MainWindow::startPhase5() {
    ...
    const QString apiKey = nvidiaApiKey();
    if (apiKey.isEmpty()) {
        m_winLogs->appendText("[ERR] Missing NVIDIA_API_KEY (or NVAPI_KEY). Cannot verify Phase 5.\n", "#ff003c");
        m_dashboard->enableEditButton(true);
        return;
    }
