...
public class PipelineRunner {
    ...
    private static String nvidiaApiKey() {
        String k = System.getenv("NVIDIA_API_KEY");
        if (k == null || k.isBlank()) k = System.getenv("NVAPI_KEY");
        if (k == null || k.isBlank()) k = System.getenv("OPENAI_API_KEY");
        if (k == null || k.isBlank())
            throw new IllegalStateException("Set NVIDIA_API_KEY (or NVAPI_KEY) for Surgeon Mind API calls.");
        return k.strip();
    }
    ...
    private String callApi(String system, String user, int maxTokens, boolean stream) throws Exception {
        ...
        var chatTemplate = new JsonObject();
        chatTemplate.addProperty("thinking", true);
        var extraBody = new JsonObject();
        extraBody.add("chat_template_kwargs", chatTemplate);
