...
class ConnectionCheckThread(QThread):
    ...
    def run(self):
        from core.nvidia_agent import DEEPSEEK_MODEL, default_chat_params_stream, nvidia_openai_client
        ...
        for attempt in range(1, 3):
            try:
                ...
                self.msleep(time_to_sleep * 1000)

        short = str(last_err)[:180] if last_err else "unknown error"
...
class FeatureExtractorPanel(QWidget):
    ...
    def on_load_local_clicked(self):
        ...
        raw_path = self.lbl_local_path.text()
        path = raw_path.replace("DIR:", "", 1).strip()
        if not path:
            self.update_readout("ERROR: Invalid local model path.")
            return
        ...
        if success:
            model_name = self.backend.model_name or os.path.basename(path) or "LOCAL_MODEL"
            self.update_readout(f"Load complete. Model '{model_name}' active.")
            self.modelLoaded.emit(model_name)
        else:
            self.update_readout("ERROR: Local model load failed.")
        self.btn_load_local.setEnabled(True)
    ...
    def _simulate_download(self):
        ...
        if self.progress >= 100:
            ...
            if success:
                ...
                self.modelLoaded.emit(self.backend.model_name or self.timer_model_id)
            else:
                self.update_readout("ERROR: HuggingFace model load failed.")
